            ...
            <section className="bg-white p-5 rounded-2xl shadow-sm border space-y-3">
              ...
              <form onSubmit={handleAddSite} className="space-y-2.5">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border space-y-2 text-xs">
                  <div className="grid grid-cols-2 gap-2 items-center"><label className="text-slate-500 font-bold">📅 계약 체결일</label><input type="date" className="bg-white border rounded-lg p-1 text-xs font-bold" value={newSiteContractDate} onChange={e => setNewSiteContractDate(e.target.value)} /></div>
                  <div className="grid grid-cols-2 gap-2 items-center"><label className="text-slate-500 font-bold">🚀 실제 착공일</label><input type="date" className="bg-white border rounded-lg p-1 text-xs font-bold" value={newSiteStartDate} onChange={e => setNewSiteStartDate(e.target.value)} /></div>
                  <div className="grid grid-cols-2 gap-2 items-center"><label className="text-slate-500 font-bold">🏁 예정 준공일</label><input type="date" className="bg-white border rounded-lg p-1 text-xs font-bold" value={newSiteEndDate} onChange={e => setNewSiteEndDate(e.target.value)} /></div>
                </div>
                <button type="submit" className="w-full bg-slate-900 text-white text-xs py-3 rounded-xl font-bold hover:bg-slate-800">신규 현장 전산 개설</button>
              </form>
            </section>

            <section className="space-y-2">
              <h3 className="text-xs font-black text-slate-400 uppercase px-1">현재 전산 가동 중인 현장 ({siteProperties.length}개)</h3>
              {siteProperties.map(s => (
                <div key={s.id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200/70 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-y-1 text-[11px] font-medium text-slate-500">
                    ...
                    <div className="col-span-2 text-[10px] text-slate-400 pt-1 font-mono">🗓️ 계약: {s.contractDate} | 착공: {s.startDate} | 준공예정: {s.endDate}</div>
                  </div>
                </div>
              ))}
            </section>
              ...
              {currentActiveSiteDetail && (
                <div className="bg-slate-900 text-slate-300 rounded-xl p-3.5 text-[11px] space-y-1 font-mono border border-slate-700">
                  <div className="text-white font-black border-b border-slate-700 pb-1 mb-1.5 flex justify-between">
                    ...
                    <span className="text-emerald-400 text-[10px]">가동 중</span>
                  </div>
